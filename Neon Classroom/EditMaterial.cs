...
namespace Neon_Classroom
{
    public partial class EditMaterial : Form
    {
        ...
        public EditMaterial(int id, TabPage scr, int classItemId, string pageName)
        {
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "")
            {
                ...
                c.Open();
                SqlCommand cm = new SqlCommand("EXEC updateMaterial  @classItemId, @title, @desc", c);
                cm.Parameters.AddWithValue("@title", textBox2.Text);
                cm.Parameters.AddWithValue("@desc", richTextBox1.Text);
                cm.Parameters.AddWithValue("@classItemId", classItemId);
                if (cm.ExecuteNonQuery() <= 0) { MessageBox.Show("Unable to Update Material", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); }
